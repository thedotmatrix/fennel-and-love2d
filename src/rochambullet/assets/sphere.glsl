...
extern float fx; // -1.0 is BARREL, 0.1 is PINCUSHION. For planets, ideally -1.1 to -4.
float fx_scale = 1.1; // Play with this to slightly vary the results.
bool dynamic_crop; // Guesses the crop amount with an admittedly badly inaccurate formula.
bool manual_crop = true; // Cut out the junk outside the sphere.
extern float manual_amount; // Higher value = more crop.
vec2 distort(vec2 p) {
  float d = length(p);
